...
function isOilSpotDef(def) {
  return String(def?.id || '').toLowerCase() === 'oilresource';
}

function createOilSpotMarker(opacityOverride) {
  ...
  ctx.shadowBlur = 8;
  ctx.strokeStyle = 'rgba(20, 190, 255, 0.95)';
  ctx.lineWidth = 6;
  ctx.beginPath();
  ...
  const marker = new THREE.Sprite(new THREE.SpriteMaterial({
    ...
    color: 0xffffff,
    transparent: true,
    opacity: opacityOverride !== null ? opacityOverride : 0.95,
    depthWrite: false
  }));
  marker.position.set(0, 0.44, 0);
  marker.scale.set(0.55, 0.55, 0.55);
  marker.userData.oilSpotMarker = true;
  return marker;
}
...
function createPieMaterial(textureName, opacityOverride, teamColorEnabled = false) {
  ...
  const missingStructureTextures = new Set([
    'page-0-scav.png',
    'page-1-icevan.png',
    'page-2-crane.png',
    'page-3-bababus.png',
    'page-4-scav.png',
    'page-34-buildings.png',
    'page-401.png',
    'page-402-scav-vtol-fac.png'
  ]);
  if (missingStructureTextures.has(texName)) {
    return new THREE.MeshLambertMaterial({ color: 0x8f8f86, transparent, opacity });
  }
...
export async function buildStructureGroup(def, rotation, sizeX, sizeY, scaleOverride = null, opacityOverride = null) {
  ...
  if (def && def.pies && def.pies.length) {
    try {
      ...
      const preserveOrigin = !!def.preserveModelOrigin;
      ...
      baseMesh.position.set(preserveOrigin ? 0 : -cX, -bb2.min.y, preserveOrigin ? 0 : -cZ);
      group.add(baseMesh);

      const alignExtrasByOrigin = !!def.alignPiesByOrigin;
      const extraPies = def.type === 'WALL' ? [] : def.pies.slice(1);
      let connectorSource = baseGeo.userData.connectors?.length
        ? {
            geo: baseGeo,
            offsetX: preserveOrigin ? 0 : -cX,
            offsetY: -bb2.min.y,
            offsetZ: preserveOrigin ? 0 : -cZ
          }
        : null;
      for (const extra of extraPies) {
        try {
          ...
          group.add(extraMesh);
          if (!connectorSource && extraGeo.userData.connectors?.length) {
            connectorSource = {
              geo: extraGeo,
              offsetX: extraMesh.position.x,
              offsetY: extraMesh.position.y,
              offsetZ: extraMesh.position.z
            };
          }
        } catch (_) {}
      }

      const connector = connectorSource?.geo.userData.connectors?.[0];
      connRel = connector
        ? {
            x: connector[0] * scl + connectorSource.offsetX,
            y: connector[2] * scl + connectorSource.offsetY,
            z: connector[1] * scl + connectorSource.offsetZ
          }
  ...
  if (isOilSpotDef(def)) {
    const marker = createOilSpotMarker(opacityOverride);
  ...
  group.userData.centerX = def.preserveModelOrigin ? 0 : center.x;
  group.userData.centerY = center.y;
  group.userData.centerZ = def.preserveModelOrigin ? 0 : center.z;
